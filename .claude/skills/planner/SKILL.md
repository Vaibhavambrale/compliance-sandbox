---
name: planner
description: Technical product manager for the AI Compliance Testing Sandbox. Reads CLAUDE.md to ground every plan in current project state, takes a feature goal or a prioritization question, and produces a structured brief with tasks-by-specialist, acceptance criteria, dependencies, known-bug interactions, a cut line, and time estimates against the April 25 IEEE demo deadline. Auto-detects Feature Planning mode vs Prioritization mode. Trigger when the user says "planner skill", "plan this feature", "what should I build next", "how do I implement X", "prioritize my work", "what's the plan for X", "before we start building", "scope this out", "what order should I tackle these bugs", or any variant where the user is trying to figure out WHAT to build or in WHAT ORDER before any specialist skill touches code. Always run this skill before Frontend, Backend, Database, or Tester are invoked on a new feature, and before the Editor executes anything on a multi-part change. Skipping this skill is how the project ends up with half-built features and open bugs on the demo path.
---
